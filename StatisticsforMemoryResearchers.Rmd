...
As per most reaction / retrieval time results, the data are highly skewed (skew = `r skew(Df1$Retrieval_Type_RT)`). A way to manage this is to simply take the logarithm of the retrieval time. The new variable is calculated and added, below. The transformation has reduced much of the skewness of the data (skew = `r skew(Df1$logRT)`).
...
where $i$ indexes different memories, $y_i$ is the natural logarithm of the retrieval time for each memory, $b_0$ and $b_1$ are the intercept and slope respectively, and $\epsilon_i$ is an error term that is assumed to be normally distributed with a mean of 0 and standard deviation $\sigma$ of 1.
