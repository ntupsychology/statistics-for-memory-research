...
$$      
where $i$ indexes different memories, each $y_i$ is the natural logarithm of the retrieval time, $b_0$ and $b_1$ are the intercept and slope, and each $e_i$ is an error term that is assumed to be Normally distributed with zero mean and standard deviation $\sigma$.

### Inline R code example
```{r}
foo <- rnorm(100)
```
The mean is `r round(mean(foo), 3)` and the variance is `r round(var(foo), 3)`.
